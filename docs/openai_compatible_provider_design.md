...
| Pydantic AI model/provider adapter | `custom_components/pydantic_ai_agent/pydantic_ai_openai_compatible/` |
...
- Home Assistant LLM tools converted through `ha_toolset.py`;
- remote Streamable HTTP MCP toolsets selected by subentry;
- selected `pydantic-ai-skills` capabilities;
- optional WebFetch;
- sliding-window context management;
- usage limits for max iterations.

## MCP Dependency Note

Remote MCP support remains Streamable HTTP only. The integration does not support
stdio, SSE, or local command MCP servers.

The dependency is `fastmcp-slim[client,server]>=3.3.0` because Pydantic AI's
`MCPToolset` imports FastMCP modules that require the server extra. Using only
`fastmcp-slim[client]` can fail at runtime before any remote MCP request is made.

## Testing

Default offline validation runs through `scripts/check` and excludes
`pytest.mark.real_server` tests.

Real-provider E2E tests live in
`tests/components/pydantic_ai_agent/test_real_server.py`. They read `.env` or
process environment values for:
...
- Keep `pyproject.toml`, `manifest.json`, `AGENTS.md`, README, and design docs in
  sync when dependency names or versions change.
- Do not add OpenAI SDK imports or requirements unless a future design explicitly
  chooses to reintroduce them.
- Preserve `reasoning` / `reasoning_content` round trips when changing message
  mapping.
- Keep real-provider test failures actionable and avoid skipping HTTP 400
  follow-up failures that indicate adapter bugs.
- Keep runtime streaming claims separate from low-level streaming support.
