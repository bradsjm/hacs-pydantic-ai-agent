...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/workspace/./nested/../file.txt", "/tmp", "/workspace/file.txt"),
        ("../file.txt", "/workspace/nested", "/workspace/file.txt"),
        ("/", "/workspace", "/"),
    ],
)
def test_normalize_vfs_path_lexically_normalizes_inside_root(
    path: str, working_directory: str, expected: str
) -> None:
...
def test_normalize_vfs_path_rejects_invalid_or_escaping_paths(
    path: str, working_directory: str
) -> None:
...
def test_protected_replacement_path_marks_only_workspace_roots(
